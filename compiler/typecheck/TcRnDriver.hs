...
data TcRnExprMode = TM_Inst    -- ^ Instantiate the type fully (:type)
                  | TM_NoInst  -- ^ Do not instantiate the type (:type-spec)
                  | TM_Default -- ^ Default the type eagerly (:type-def)
...
:type-spec / TM_NoInst
  ...
  solved. Can be abbreviated to :ts
...
:type-def / TM_Default
  ...
  defaulted. Can be appreviated to :td
